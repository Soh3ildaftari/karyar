...
class player(possision,status):
    def __init__(self, x, y, max_width, max_hight,name, ms=1):
        ...
        status.__init__(self,name,hp=100,state='live')
    def hiited(self):
        if self.hp<0:
            self.hp-=10
        else:
...
player = player(100,100,WIDTH,HEIGHT/2,"reza",)  
shuts=list()
# Game loop  
while True:  
    ...
    if keys[pygame.K_SPACE]:  
        shout_instance = player.shout()  
        if isinstance(shout_instance,shout):  
            shuts.append(shout_instance)
    screen.fill(WHITE)  
